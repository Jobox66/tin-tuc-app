 ...
 *
 * Tất cả các đường dùng CHUNG một đơn vị VNĐ/chỉ, kể cả giá thế giới (đã quy đổi
 * từ USD/oz theo tỷ giá của chính ngày đó). Nhờ vậy không cần trục thứ hai và
 * không cần quy về chỉ số - đọc thẳng được chênh lệch trong nước so với thế giới.
 * Giá USD gốc vẫn hiện trong tooltip và bảng số.
 *
 * Palette: 4 slot categorical đã chạy validator, pass cả light lẫn dark
 * (CVD ΔE 9.1 light / 8.4 dark, normal-vision 22.9 / 19.8). Light mode có 2 màu
 * dưới ngưỡng contrast 3:1 nên bắt buộc kèm nhãn trực tiếp + bảng số.
 */

export interface ChartPoint {
    day: string;
    value: number;        // VNĐ/chỉ - giá trị được vẽ
    usd?: number;         // USD/oz - chỉ có ở đường giá thế giới
}
...
export interface ChartSeries {
    ...
    label: string;
    color: "s1" | "s2" | "s3" | "s4";
    dashed?: boolean;     // đường quy đổi vẽ nét đứt để phân biệt với giá niêm yết
    points: ChartPoint[];
}
...
export default function GoldPriceChart({ series, days }: GoldPriceChartProps) {
    ...
    const chart = useMemo(() => {
        ...
        const plotted = series.map(s => {
            const byDay = new Map(s.points.map(p => [p.day, p]));
            return {
                ...s,
                values: days.map(d => {
                    const p = byDay.get(d);
                    return p && p.value > 0 ? p.value : null;
                }),
                usds: days.map(d => byDay.get(d)?.usd ?? null),
            };
        });
        ...
        const withGeometry = plotted.map(s => {
            ...
            return {
                ...
                path: segs.join(" "),
                lastIdx,
                pointCount: s.values.filter(v => v !== null).length,
            };
        });
        ...
        return { plotted: withGeometry, ticks, x, y, xLabels };
    }, [series, days]);
                    ...
                    <span key={s.key} className="flex items-center gap-2 text-zinc-600 dark:text-zinc-400 font-medium">
                        <svg width="22" height="4" aria-hidden="true">
                            <line
                                ...
                                stroke={COLOR_VAR[s.color]} strokeWidth="2" strokeLinecap="round"
            ...
            <div className="overflow-x-auto">
                <div className="relative" style={{ minWidth: 640 }}>
                    <svg
                        ...
                        role="img"
                        aria-label="Biểu đồ giá vàng trong nước và giá thế giới quy đổi, theo ngày"
                    >
                        {ticks.map(t => (
                            <g key={t}>
                        ...
                        {plotted.map(s => (
                            <path
                                key={s.key} d={s.path} fill="none" stroke={COLOR_VAR[s.color]}
                                    ...
                                    <div key={`t-${s.key}`} className="flex items-center justify-between gap-4 mb-1 last:mb-0">
                                        <span className="flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400">
                                            <svg width="14" height="4" aria-hidden="true">
                                                <line
                                                    x1="0" y1="2" x2="14" y2="2" stroke={COLOR_VAR[s.color]}
                                        ...
                                        <span className="font-bold text-zinc-900 dark:text-zinc-100 tabular-nums text-right">
                                            {fmtVnd(v)}
                                            {usd !== null && usd !== undefined && (
                                                <span className="block text-[11px] font-medium text-zinc-500 dark:text-zinc-400">
                                                    ${usd.toLocaleString("en-US")}/oz
                                                </span>
                                            )}
                                        </span>
                                    </div>
